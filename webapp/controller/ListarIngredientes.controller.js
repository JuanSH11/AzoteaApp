sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.sap.build.standard.azoteaApp.controller.ListarIngredientes", {
				...
				.then(function(result) {
					if (result === false) {
						...
						return new Promise(function(fnResolve) {
							...
							sap.m.MessageToast.show("Ingrediente eliminado", {
								onClose: fnResolve,
								duration: 2000 || 3000,
								at: sTargetPos,
								my: sTargetPos
			...
			if (status) {
				...
				return new Promise(function(fnResolve, fnReject) {
					...
					if (bHasPendingChanges) {
						...
						var oNewEntityInstance = Utilities.getDefaultValuesForCrearIngrediente();

						oModel = this.getView().getModel();
						var oNewBindingContext = oModel.createEntry("IngredienteSet", {
							properties: oNewEntityInstance
						});

						this.doNavigate("CrearIngrediente", oNewBindingContext, fnResolve);
					}
				}.bind(this)).catch(function(err) {
					if (err !== undefined) {
						MessageBox.error(err.message);
		...
		onInit: function() {
			...
			this.oRouter.getTarget("ListarIngredientes").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
